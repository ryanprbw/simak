<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <select id="departement" name="departement" class="mt-1 block w-full">
        ...
        <div>
            ...
            <select id="education" name="education" class="mt-1 block w-full">
                @foreach (['High School', 'SD / Sederajat', 'SMP / Sederajat', 'SMA / Sederajat', 'D3', 'S-1', 'S-2', 'S-3'] as $edu)
                    <option value="{{ $edu }}" {{ $user->education == $edu ? 'selected' : '' }}>
                        {{ $edu }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="field" name="field" class="mt-1 block w-full">
                @foreach (['Sekretariat', 'Pelayanan Pendaftaran Penduduk', 'Pengelolaan Informasi Administrasi Kependudukan', 'Pelayanan Pencatatan Sipil', 'Pemanfaatan Data dan Inovasi Pelayanan'] as $field)
                    <option value="{{ $field }}" {{ $user->field == $field ? 'selected' : '' }}>
                        {{ $field }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="position" name="position" class="mt-1 block w-full">
                @foreach (['Kepala Bidang', 'Kepala Dinas Kependudukan dan Pencatatan Sipil', 'Sekretaris Dinas Kependudukan dan Pencatatan Sipil', 'Kepala Bidang Pelayanan Pendaftaran Penduduk', 'Kasubbag Keuangan', 'Pengawas Kependudukan', 'Administrator Database Kependudukan'] as $pos)
                    <option value="{{ $pos }}" {{ $user->position == $pos ? 'selected' : '' }}>
                        {{ $pos }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="roles" name="roles" class="mt-1 block w-full">
        ...
        <div>
            ...
            <select id="eselon" name="eselon" class="mt-1 block w-full">
